...
export default function PortraitGeneratorForm() {
  ...
  const handleCreateNew = () => {
    if (analytics) {
      logEvent(analytics, 'create_new_portrait_click');
    }
  ...
  const handleDownload = async () => {
    ...
    if (analytics) {
        logEvent(analytics, 'download_portrait_click');
    }
    try {
  ...
  const handleShare = async () => {
      if (analytics) {
        logEvent(analytics, 'share_app_click');
      }
      try {
  ...
  const handleShopTheLook = () => {
    ...
    if (!hatStyle) return;
    if (analytics) {
      logEvent(analytics, 'shop_the_look_click', { hat_style: hatStyle });
    }
    
    const amazonTag = process.env.NEXT_PUBLIC_AMAZON_ASSOCIATE_TAG;
    if (!amazonTag) {
        console.error("Amazon Associate Tag is not configured.");
        toast({ title: 'Shopping Error', description: 'The shopping link could not be generated.', variant: 'destructive' });
        return;
    }
  ...
  const handleGenerationSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    if (analytics) {
      if (customHat.trim()) {
        logEvent(analytics, 'generate_portrait_custom_hat', { hat_style: customHat.trim() });
      } else {
        logEvent(analytics, 'generate_portrait_standard_hat', { hat_style: selectedHat });
      }
    }
